...
class Predinormalize(object):
    ...
    def __call__(self, tensor):
        #print(tensor.shape)
#        tensor_pro=torch.nn.Softmax(dim=1)(tensor.clone())
        tensor_pro=tensor*2-1
        return tensor_pro
...
class kerel_viz(object):
    def __call__(self, np_image):
        #print(np_image)
        feature=(np_image- np_image.min())/ (np_image.max()-np_image.min())
        kernel = np.uint8(feature*255)
        #print(kernel)
        return kernel

class dis_viz(object):
    def __call__(self, np_image):
        np_image=np.clip(np_image,-20,20)
        feature = (np_image - np_image.min()) / (np_image.max() - np_image.min())
        feature = np.uint8(feature*255)
        return feature

class Co_viz():
    def __call__(self, learned,target_dis):
        sdf=np.zeros((learned.shape[1],learned.shape[2]))
        learned_sdf=np.zeros((learned.shape[1],learned.shape[2]))
        for index in range(target_dis.shape[0]):
            sdf = np.where(target_dis[index] != 1000, target_dis[index],sdf)
            learned_sdf=np.where(target_dis[index] != 1000, learned[index],learned_sdf)
        sdf =np.clip(sdf ,-20,20)
        field= (sdf  - sdf .min()) / (sdf .max() - sdf .min())
        field = np.uint8(field*255)

        learned_sdf =np.clip(learned_sdf ,-20,20)
        learned_sdf= (learned_sdf -learned_sdf.min()) / (learned_sdf.max() - learned_sdf.min())
        learned_sdf = np.uint8(learned_sdf*255)

        return learned_sdf,field

class sdf_viz():
    def __call__(self, np_image):
        sdf=np.zeros((np.shape[1],np.shape[2]))
        for index in np_image:
            sdf = np.where(np_image[index] != 1000, np_image[index],sdf)
        sdf =np.clip(sdf ,-20,20)
        field= (sdf  - sdf .min()) / (sdf .max() - sdf .min())
        field = np.uint8(field*255)
        return field
