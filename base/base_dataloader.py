...
class DataPrefetcher(object):
    ...
    def preload(self):
        try:
            self.next_input, self.next_target,self.next_distance = next(self.loaditer)
        ...
        with torch.cuda.stream(self.stream):
            ...
            self.next_distance = self.next_distance.cuda(device=self.device, non_blocking=True)
    def __iter__(self):
        count = 0
