...
class BaseTrainer:
    def __init__(self, model, loss, resume, config, train_loader, val_loader=None, train_logger=None):
        ...
        if isinstance(self.model, torch.nn.DataParallel):
            kernel_params = [{'params':filter(lambda p:p.requires_grad, self.model.module.get_kernel_params()),
                                    'lr': 0.007,
                              'eps':1e-10}]

            back_params = [{'params': filter(lambda p: p.requires_grad, self.model.module.get_backbone_params())},
                           {'params': filter(lambda p: p.requires_grad, self.model.module.get_decoder_params()),
                            'lr': 1e-4,
                            'eps':1e-8}]
        else:
            kernel_params = [{'params': filter(lambda p: p.requires_grad, self.model.get_kernel_params()),
                              'lr': 1e-5,
                              'eps':1e-10}]

            back_params = [{'params': filter(lambda p: p.requires_grad, self.model.get_backbone_params())},
                           {'params': filter(lambda p: p.requires_grad, self.model.get_decoder_params()),
                            'lr': 1e-4,
                            'eps':1e-8}]
        self.kernel_optimizer = get_instance(torch.optim, 'optimizer', config, kernel_params)
        self.back_optimizer = get_instance(torch.optim, 'optimizer', config, back_params)

        self.kernel_lr_scheduler = getattr(utils.lr_scheduler, config['lr_scheduler']['type'])\
            (self.kernel_optimizer, self.epochs, len(train_loader))
        self.back_lr_scheduler = getattr(utils.lr_scheduler, config['lr_scheduler']['type'])\
            (self.back_optimizer,self.epochs,len(train_loader))
        # MONITORING
        ...
        if resume: self._resume_backone(resume)
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        state = {
            ...
            'kernel_optimizer': self.kernel_optimizer.state_dict(),
            'back_optimizer': self.back_optimizer.state_dict(),
    ...
    def _resume_backone(self,resume_path):
        ...
        self.model.module.back_arch.load_state_dict(new_state_dict)
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        if checkpoint['config']['arch'] != self.config['arch']:
            self.logger.warning({'Warning! Current model is not the same as the one in the checkpoint'})
        self.model.load_state_dict(checkpoint['state_dict']['module'])
