...
class VOCDataset(BaseDataSet):
    ...
    def _load_data(self, index):
        ...
        image = np.asarray(Image.open(image_path).resize((self.size,self.size)), dtype=np.float32)
        label = np.asarray(Image.open(label_path).resize((self.size,self.size)), dtype=np.int32)
        distance=np.asarray(Distance_transfer(label_path,self.num_classes,self.size),dtype=np.float32)
        #print(distance)
